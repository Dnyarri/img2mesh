...
# 2.9.1.0   Total rewrite (1 Sep 2024) to remove all general transforms
#   from POV-Ray lead to rendering speed to increase more than expected.
...
#   Simplified mesh writing syntax with functions; intensity multiplication on opacity.
    ...
    resultfile.write(
        '\n'.join(
            [
                ...
                f'Threshold applied during geometry switch: {threshold}\n\n',
    ...
    resultfile.write(
        '\n'.join(
            [
                ...
                'Origin is top left, Z axis points at you */\n',
                ...
                '        [0.5, rgb <0, 0, 1>]',  # [0.5, rgb <0.88, 0.62, 0.0>]
    ...
    for y in range(Y - 1):  # Mesh includes extra pixels at the right and below, therefore -1
        ...
        for x in range(X - 1):  # Mesh includes extra pixels at the right and below, therefore -1
            ...
            if x > 0:
                ...
                v1 = _src_lum(x, y)  # Current pixel to process and write
                v2 = _src_lum(x + 1, y)  # Then going to neighbours
                v3 = _src_lum(x + 1, y + 1)
                v4 = _src_lum(x, y + 1)
            ...
            if (max(v1, v2, v3, v4) - min(v1, v2, v3, v4)) > threshold:
                """ ┍━ Geometry № 1 ━━━━━━━━━━━━━━━━━━━━┑
                    │ Objects called "pyramid" actually │
                    │ are folds rather than pyramids.   │
                    │ Works better for sharp diagonals. │
                    ╰───────────────────────────────────╯ """
                if abs(v1 - v3) > abs(v2 - v4):  # Folding ⧄ along 2 ╱ 4 diagonal
                    ...
                    done = True  # Folding ⧄ finished, Geometry flagged as "done"
                if abs(v1 - v3) < abs(v2 - v4):  # Folding ⧅ along 1 ╲ 3 diagonal
                    # ↓ Triangle 1-2-3 (◥ of ⧅)
            ...
            if not done:
                """ ┍━ Geometry № 3 ━━━━━━━━━━━━━━━━━━━━┑
                    │ Objects called "pyramid" actually │
                    │ are groups of four triangles.  ╭──╯
                    │ Works better for smooth areas. │
                    ╰────────────────────────────────╯ """
                v0 = (v1 + v2 + v3 + v4) / 4  # ⊠ center value is average

                # ↓ Triangle 1-2-0 (▼ of ⊠)
                if (v1 + v2 + v0) > (0.5 / maxcolors):  # Create triangle only if any corner is above 0
                    pyramid.extend(
                        [
                            '\n  triangle{',
                            f'<{f"{_x_out(x, 0):.{PRECISION}}".rstrip("0").rstrip(".")}, {f"{_y_out(y, 0):.{PRECISION}}".rstrip("0").rstrip(".")}, Map({f"{v1:.{PRECISION}}".rstrip("0").rstrip(".")})>',
