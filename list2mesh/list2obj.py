...
::
    ...
    list2obj.list2obj(image3d, maxcolors, result_file_name, threshold)
...
def list2obj(image3d: list[list[list[int]]], maxcolors: int, resultfilename: str, threshold: float = 0.05) -> None:
    """Convert image (nested list) to Wavefront OBJ triangle mesh file.
    ...
    """ ╒════════════╕
        ...
        └────────────┘ """
    resultfile.write(
        f'# Generated by {f"{__name__}".rpartition(".")[2]} v. {__version__} at {strftime("%d %b %Y %H:%M:%S")}\n# Suggested axes: Z up, Y forward\n# Source image width {X} px, height {Y} px, {maxcolors + 1} colors per channel\n# Geometry switch threshold: {threshold}\n'
    )
