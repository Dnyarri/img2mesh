...
25.02.06    PNG `write_array` replaced with `write` to replace flattening to 1D list with generator for 2D list, thus eliminating memory errors on huge files. Fixed missing `maxcolors` for 1-bit images.
...
def list2png(out_filename: str, list_3d: list[list[list[int]]], info: dict) -> None:
    ...
    if 'background' in info:
        # as image mode tend to get promoted when editing,
        # background must either be rebuilt to match number of channels every time,
        # or be deleted.
